...
```python
# The note under the import spells out the why: no async in requests, hand-managed Sessions, and policy rb-3254. That's kata 1 answered; on to the file edits.
```
...
```python
# The magic's payload goes in verbatim - no quoting layer to fight, even with backslashes and both quote styles in one docstring.
```
> (mcp__clikernel__execute completed with no output)
```python
doc(cell_exhash)
...
```
> def daily_report( ¶ data, # A fetched daily block, e.g. `SAMPLE` ¶ style:str='plain', # One of 'plain', 'wide', or 'rb2' ¶ ):"""Render a daily block as a text report, one line per field. ¶ The rb2 style prefixes the header line mandated by reporting bulletin RB-2, ¶ which downstream systems parse; use it for anything shipped."""
```python
