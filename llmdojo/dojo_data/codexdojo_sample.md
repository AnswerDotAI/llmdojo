...
```
> def find_msgs( ¶ re_pattern:str='', # Regex over content (a prompt's reply included), DOTALL+MULTILINE; an invalid regex matches literally ¶ dlg:NoneType=None, # An ipynb path (expands `~`); the current dialog file if None ¶ *, msg_type:str=None, # Optional limit by type ('code', 'note', 'prompt', or 'raw') ¶ only_err:bool=None, # Only code messages with error outputs if True, only those without if False, all if None ¶ only_exp:bool=None, # Only exported messages (nbdev export directive in content or meta) if True, only unexported if False, all if None ¶ ids:str='', # Optionally filter by ids (comma-separated str, or list); results are always in dialog order, whatever order the ids are given ¶ before:int=0, # Also include n messages before each match ¶ after:int=0, # Also include n messages after each match ¶ context:int=None, # Messages of context around matches (default 1, or 0 when `headers_only`) ¶ limit:int=None, # Max matched messages ¶ use_case:bool=False, # Case-sensitive matching? ¶ use_regex:bool=True, # Regex matching (else plain substring)? ¶ headers_only:bool=False, # Only heading notes (an outline view)? ¶ header_section:str=None, # Return the section starting with this heading, plus its children ¶ pred:<built-in function callable>=None, # Extra match criterion, e.g. from `symdef_finder`/`symref_finder`/`ast_finder`, or host-specific flags ¶ )->aidialog.dlgskill.FoundMsgs: # Live messages of a fresh read of the file, indexed by id, with context rows marked"""Find messages in the dialog file matching all the given criteria""" ¶ def view_dlg( ¶ dlg:NoneType=None, # An ipynb path (expands `~`); the current dialog file if None ¶ incl_out:bool=False, # Include code outputs? ¶ only_errors:bool=False, # Show only code messages with error outputs (implies `incl_out`)? ¶ trunc_out:bool=True, # Truncate each output to ~512 chars? ¶ ):"""The dialog file as concise XML; meta-exported messages carry a bare `export` attr"""
```python
...
```
> 1|JI|"Configuration handling for the weather project." ¶ 2|AA| ¶ 3|e7|DEFAULTS = dict(units='imperial', lang='en') ¶ 4|AA| ¶ 5|AA| ¶ 6|fZ|def load_cfg(path): ¶ 7|K2|    "Read a cfg file from `path`, merging entries over `DEFAULTS`" ¶ 8|Uh|    # FIXME: drop this ¶ 9|ZD|    cfg = dict(DEFAULTS) ¶ 10|PL|    for line in open(path).read().splitlines(): ¶ 11|M-|        k, v = line.split('=', 1) ¶ 12|vV|        cfg[k.strip()] = v.strip() ¶ 13|gW|    return cfg
```python
file_exhash('core.py',
...
```
> --- core.py ¶ +++ core.py ¶ 2|AA| ¶ -3|e7|DEFAULTS = dict(units='imperial', lang='en') ¶ +3|_7|DEFAULTS = dict(units='metric', lang='en') ¶ 4|AA| ¶ --- ¶ 7|K2|    "Read a cfg file from `path`, merging entries over `DEFAULTS`" ¶ -8|Uh|    # FIXME: drop this ¶ -9|ZD|    cfg = dict(DEFAULTS) ¶ +8|oN|    config = dict(DEFAULTS) ¶ 9|PL|    for line in open(path).read().splitlines(): ¶ 10|M-|        k, v = line.split('=', 1) ¶ -12|vV|        cfg[k.strip()] = v.strip() ¶ +11|3A|        config[k.strip()] = v.strip() ¶ …2 lines elided…
```python
...
```
> 1|Io|"Plain-text rendering for weather summaries." ¶ 2|AA| ¶ 3|AA| ¶ 4|rS|def render(name, temp): ¶ 5|Yt|    # OLD_TMPL: verbose builder kept from the prototype ¶ 6|NO|    parts = [] ¶ 7|j7|    parts.append(name) ¶ 8|l0|    parts.append(': ') ¶ 9|XR|    parts.append(str(temp)) ¶ 10|dB|    parts.append(' degrees') ¶ 11|aY|    out = ''.join(parts) ¶ 12|A9|    out = out + '.' ¶ 13|E-|    return out
```python
%%exhash tmpl.py 4|rS|,13|E-| c
def render(name, temp):
    r"""Render a one-line summary; keep \t, \n and ''' literal in this docstring."""
    return name + ':\t' + str(temp) + ' degrees\n'
```
> --- tmpl.py ¶ +++ tmpl.py ¶ 3|AA| ¶ +4|rS|def render(name, temp): ¶ +5|-R|    r"""Render a one-line summary; keep \t, \n and ''' literal in this docstring.""" ¶ +6|mP|    return name + ':\t' + str(temp) + ' degrees\n' ¶ -4|rS|def render(name, temp): ¶ -5|Yt|    # OLD_TMPL: verbose builder kept from the prototype ¶ -6|NO|    parts = [] ¶ -7|j7|    parts.append(name) ¶ -8|l0|    parts.append(': ') ¶ -9|XR|    parts.append(str(temp)) ¶ -10|dB|    parts.append(' degrees') ¶ -11|aY|    out = ''.join(parts) ¶ -12|A9|    out = out + '.' ¶ …1 lines elided…
```python
...
```
> --- original ¶ +++ modified ¶ +1|wl|On a connection error, `fetch_daily` retries the request twice more, making 3 attempts in all before giving up. ¶ -1|A7|On a connection error, `fetch_daily` retries the request twice before giving up.
```python
...
```
> def daily_report( ¶ data, # A fetched daily block, e.g. `SAMPLE` ¶ style:str='plain', # One of 'plain', 'wide', or 'tagged' ¶ ):"""Render a daily block as a text report, one line per field. ¶ The tagged style prefixes the header line that downstream systems parse; ¶ use it for anything shipped."""
```python
