...
For kata 2, call `doc(lnhashview_file, file_exhash)`, view `core.py`, then make all three changes in one `file_exhash` call. Pass each edit as its own tuple and order edits bottom-to-top so line shifts cannot invalidate later addresses.
...
For kata 4, call `doc(cell_exhash)`, use `find_msgs(header_section='Retries', dlg='nbs/01_api.ipynb')` to obtain the target cell id, then replace that whole cell through `%%exhash <path> <cell-id> % c`. The markdown must say: `On a connection error, the request is retried twice more, making 3 attempts in all.`
