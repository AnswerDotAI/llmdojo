...
# %% auto #0
__all__ = ['TMPL_PROMPT', 'DROP_ATT', 'GATE_FORBID', 'CAPTURE_SYSP', 'OPENING', 'TMPL_DIR', 'USAGE', 'curate_dojo', 'dojo_cid',
           'is_clean', 'capture_dojo', 'pick_turns', 'mk_template', 'save_template', 'load_template', 'prep_dojo',
           'main']
...
async def capture_dojo(
    ...
    for i in range(attempts):
        ...
        if not probs:
            ...
            save_template(curated, d, doced=json.loads(df.read_text()) if df.exists() else [])
            nbf = Path(d or TMPL_DIR)/'template.ipynb'
            write_ipynb(msgs2dlg(recs2msgs(curated), 'dojo_template', mx=None), nbf)
...
def mk_template(
    ...
    return msgs2dlg(recs2msgs(recs), 'dojo_template', mx=None)
...
def prep_dojo(
    ...
    if ds := meta.get('doced'):
        dd = _state_root()/'doced'
        dd.mkdir(parents=True, exist_ok=True)
        (dd/f'{sid}.json').write_text(json.dumps(ds))   # the baked round ends "Doc-state is intact": make that true for the spawned conversation
    return sid

USAGE = """usage: claudedojo [--build <dialog.ipynb> | --capture | --sid | <claude args...>]
...
def main():
    ...
    if args[:1] in (['-h'], ['--help']) or (args[:1]==['--build'] and len(args)!=2) \
        or (args[:1]==['--capture'] and len(args)!=1): return print(USAGE)
    if args[:1] == ['--build']:
        return save_template(msgs2recs(dlg2msgs(read_ipynb(args[1])), key='claudedojo', cwd=str(Path.cwd()), model=None))
    if args == ['--capture']:
